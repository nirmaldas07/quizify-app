...
export default function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const startedQuizzes = eventsList.filter(e => e.buttonName === 'start_quiz').length;
      const completedQuizzes = quizList.length;
      const completionRate = startedQuizzes > 0 ? Math.round((completedQuizzes / startedQuizzes) * 100) : 0;
      ...
      // Calculate retention data (D1-D7)
  ...
  const calculateRetention = (users, sessions) => {
    ...
    retentionDays.forEach(day => {
      ...
      users.forEach(user => {
        ...
        // Only count users who joined at least 'day' days ago
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="px-6 py-4 bg-gray-900/50 border-b border-gray-800">
        <div className="flex gap-3 overflow-x-auto pb-2">
          <div className="flex-shrink-0 px-4 py-2 bg-gray-800 rounded-lg">
            <div className="text-xs text-gray-400">Sessions</div>
            <div className="text-lg font-bold">{stats.totalSessions}</div>
          </div>
          <div className="flex-shrink-0 px-4 py-2 bg-gray-800 rounded-lg">
            <div className="text-xs text-gray-400">Avg Duration</div>
            <div className="text-lg font-bold">{formatDuration(stats.avgSessionDuration)}</div>
          </div>
          <div className="flex-shrink-0 px-4 py-2 bg-gray-800 rounded-lg">
            <div className="text-xs text-gray-400">Quizzes</div>
            <div className="text-lg font-bold">{stats.totalQuizzes}</div>
          </div>
          <div className="flex-shrink-0 px-4 py-2 bg-gray-800 rounded-lg">
            <div className="text-xs text-gray-400">Quiz/User</div>
            <div className="text-lg font-bold">{stats.quizzesPerUser}</div>
          </div>
          <div className="flex-shrink-0 px-4 py-2 bg-gray-800 rounded-lg">
          ...
          <div className="flex-shrink-0 px-4 py-2 bg-gray-800 rounded-lg">
            ...
            <div className="text-lg font-bold">{stats.bounceRate}%</div>
          </div>
          <div className="flex-shrink-0 px-4 py-2 bg-gray-800 rounded-lg">
            <div className="text-xs text-gray-400">Growth</div>
            <div className={`text-lg font-bold ${stats.dailyGrowth >= 0 ? 'text-green-400' : 'text-red-400'}`}>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
                {stats.topButtons.map((button, idx) => (
                  <div key={idx} className="bg-gray-800 rounded-lg p-3 text-center">
                    <div className="text-xl mb-1">{idx === 0 ? '🥇' : idx === 1 ? '🥈' : idx === 2 ? '🥉' : '📊'}</div>
                    <div className="text-sm font-medium truncate">{button.name}</div>
                    <div className="text-xs text-gray-400">{button.count} clicks</div>
                  </div>
                ))}
              </div>
            </div>
                ...
                <tbody>
                  {users.map(user => (
                    <tr key={user.id} className="border-b border-gray-800 hover:bg-gray-900/50">
                      <td className="py-3 px-4 font-medium">{user.userId || user.id}</td>
                      ...
                      <td className="py-3 px-4 text-gray-400">{formatDate(user.lastActiveDate)}</td>
        ...
        {activeTab === 'engagement' && (
          <div className="space-y-6">
            ...
            <div className="bg-gray-900 rounded-xl p-6">
              ...
              {/* Retention Chart */}
              <h4 className="text-md font-semibold mb-3">Retention by Day</h4>
        ...
        >
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4 text-sm mb-4">
              <div><span className="text-gray-400">User ID:</span> {selectedUser.userId || selectedUser.id}</div>
              <div><span className="text-gray-400">Sessions:</span> {selectedUser.totalSessions || 0}</div>
              <div><span className="text-gray-400">Quizzes:</span> {selectedUser.totalQuizzes || 0}</div>
              <div><span className="text-gray-400">Score:</span> {selectedUser.totalScore || 0}</div>
            </div>
            ...
            <button
              ...
              className="mt-4 px-4 py-2 bg-gray-800 hover:bg-gray-700 rounded-lg w-full"
            >
              Close
            </button>
          </div>

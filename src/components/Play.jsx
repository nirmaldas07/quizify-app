...
function Sheet({ open, onClose, children }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/50" onClick={onClose}>
      <div
        className="absolute inset-x-0 bottom-0 mx-auto w-full max-w-md rounded-t-3xl bg-base-card border border-base-border p-5 pb-[calc(1rem+env(safe-area-inset-bottom))]"
...
export default function Play() {
  ...
  const spin = () => {
    ...
    // Because rotation is clockwise, final visual angle of the slice is (center - rotation).
    // We want that to equal pointerAngle.
    // => center - (current + delta) ≡ pointerAngle  (mod 360)
    // => delta ≡ center - current - pointerAngle

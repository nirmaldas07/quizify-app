...
export const initializeFirebase = async () => {
  if (!Capacitor.isNativePlatform()) {
    console.log('Not on native platform, skipping Firebase native init');
    return true;
  }
  
  try {
    // Initialize Firebase Authentication for Capacitor
    console.log('Initializing Firebase Authentication...');
    
    // The Capacitor plugin uses the native Firebase SDK
    // which is configured via google-services.json
    // No explicit initialization needed for newer versions
    
    // Test if Firebase is working by getting current user
    const user = await FirebaseAuthentication.getCurrentUser();
    console.log('Firebase initialized. Current user:', user ? user.uid : 'none');
    
    return true;
  } catch (error) {
    console.error('Firebase initialization check:', error);
    // Even if getCurrentUser fails, Firebase might still be working
    // This can happen if no user is signed in
    return true;
  }
};
...
export const verifyOTP = async (code, verificationId = null) => {
  try {
    ...
    // For Capacitor, we need to complete the sign-in with the verification code
    const result = await FirebaseAuthentication.signInWithPhoneNumber({

...
// Smart Capacitor loading - works everywhere
let CapacitorApp;
try {
  // Try to load real Capacitor
  const cap = require('@capacitor/app');
  CapacitorApp = cap.App;
} catch {
  // If Capacitor isn't available (web), use mock
  CapacitorApp = {
    addListener: () => ({ remove: () => {} }),
    minimizeApp: () => {},
    exitApp: () => {}
...
function GameProvider({ children }) {
  ...
  // Merge both game states, with gameData taking precedence for coins/XP
  const mergedState = {
    ...gameState,

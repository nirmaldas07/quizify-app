//src/components/Swipe/SwipeQuiz.jsx
...
// Don't extract addCoins or addXP - use GameDataService directly
  ...
  const [coinsAlreadyAdded, setCoinsAlreadyAdded] = useState(false);
...
useEffect(() => {
  ...
  const handleAnswer = (questionId, optionIndex) => {
    ...
    if (isCorrect) {
      ...
      // Add coins to GameDataService immediately
      GameDataService.addCoins(2, 'Swipe Quiz Correct Answer');
      // Update display
      const newTotal = GameDataService.getCoins();
      setDisplayCoins(newTotal);
      
    // Show coin animation and fly to pill
      setShowCoinGain(true);
      setTimeout(() => {
        playCoin();
        
      // Trigger coin fly animation
      console.log('Coin pill ref:', coinPillRef.current);
      if (coinPillRef.current) {
        setCoinFly({ 
          ...
          count: 2,  // 2 coins animation
          amount: 2   // 2 coins value
        });
      } else {
        console.log('Coin pill ref not available');
      }
      }, 300);
...
// Update display coins from GameDataService
  useEffect(() => {
...
useEffect(() => {
  ...
  // Trigger coin animation ONLY ONCE
  if (sessionCoins > 0 && coinPillRef.current && !coinsAlreadyAdded) {
    setCoinsAlreadyAdded(true);
    setTimeout(() => {
      setCoinFly({ 
        startRect: { 
          left: window.innerWidth / 2 - 50, 
        ...
        count: Math.min(sessionCoins, 10),
        amount: sessionCoins
      });
      playCoin();
    }, 100);
  ...
  const timer = setTimeout(() => {
    // Coins have already been added during gameplay, don't add again
    // Just update the display to ensure it's current
    const currentTotal = GameDataService.getCoins();
    setDisplayCoins(currentTotal);
...
}, [sessionCoins, coinsAlreadyAdded]); // Add dependencies
    
return (
  <div className="fixed inset-0 bg-gray-900 z-50">
    {/* Coin Animation for Results Screen */}
    {coinFly && (
      <CoinFly
        ...
        count={coinFly.count}
        onDone={() => {
          // Don't add coins here - they were already added
          setCoinFly(null);
        }}
      />
    )}
    ...
    <div className="h-screen bg-gray-900 relative">
      ...
      {coinFly && (
        <CoinFly
          ...
          count={coinFly.count}
          onDone={() => {
            // Don't add coins here - they should be added only once per correct answer
...
{/* Coin Gain Animation */}
{showCoinGain && (
  <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-[150] pointer-events-none">
    <div className="bg-yellow-500 text-white px-6 py-3 rounded-full font-bold animate-coinFloat">
      +2 🪙
    </div>
  </div>
)}

    ...
    <div className="fixed inset-0 pointer-events-none z-50">
      {[...Array(count)].map((_, i) => (
        <div
          ...
          style={{
            left: startRect.left,
            top: startRect.top,
            animation: `coinFly 1.5s ${i * 0.1}s ease-out forwards`,
          }}
        >
          🪙
        </div>
      ...
      <style>{`
        @keyframes coinFly {
          ...
          100% { transform: translate(200px, -200px) scale(0.3); opacity: 0; }
        }
      `}</style>
    </div>
  );
...
export default function Home() {
  ...
  const [dailyQ, setDailyQ] = useState(sampleQuestions[0]);
  ...
  const handleDailyAnswer = (idx, e) => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        // Auto-advance to next question prompt
        setTimeout(() => {
  ...
  const loadNextQuestion = () => {
    ...
    const nextIndex = (questionIndex + 1) % sampleQuestions.length;
    setQuestionIndex(nextIndex);
    setDailyQ(sampleQuestions[nextIndex]);
  ...
  const quickActions = [
    { icon: "🎯", label: "Daily Quest", color: "bg-gradient-to-br from-orange-500 to-pink-500" },
    { icon: "🔥", label: `${streak} Streak`, color: "bg-gradient-to-br from-red-500 to-orange-500" },
    { icon: "🏆", label: "Achievements", color: "bg-gradient-to-br from-blue-500 to-purple-500" },
    { icon: "🎁", label: "Rewards", color: "bg-gradient-to-br from-green-500 to-teal-500" },
  ];
  ...
  return (
    <div className="min-h-screen bg-base-bg text-white">
      ...
      <div className="relative z-10 px-4 pt-10 pb-24">
        ...
        {/* Enhanced Daily Challenge with Carousel */}
        <div className="rounded-3xl bg-white/5 border border-white/10 overflow-hidden mb-6">
          <div className="relative h-[280px]">
            {/* Question Screen */}
            {currentScreen === 'question' && dailyQ && (
              <div className={`absolute inset-0 p-6 ${animating ? 'slide-out' : ''}`}>
                ...
                <p className="text-sm mb-4 opacity-70">{dailyQ.prompt}</p>
                ...
                {picked === dailyQ?.answerIndex ? (
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-base font-medium bg-gradient-to-r from-green-400 to-emerald-400 bg-clip-text text-transparent">
                      {dailyQ?.options[dailyQ.answerIndex]}
                    </p>
                  </div>
                )}
              </div>
            ...
            {currentScreen === 'funfact' && (
              <div className="absolute inset-0 p-6 flex flex-col items-center justify-center slide-in bg-gradient-to-br from-purple-500/20 to-pink-500/20">
                ...
                <div className="absolute top-4 right-4 text-xs opacity-50">Auto-advancing...</div>
              </div>
            )}
            ...
            {currentScreen === 'next' && (
              <div className="absolute inset-0 p-6 flex flex-col items-center justify-center slide-in">
                <div className="text-5xl mb-4" style={{ animation: 'pulse 1s infinite' }}>🚀</div>
                <h3 className="text-xl font-bold mb-3">Ready for more?</h3>
                <button
                  onClick={loadNextQuestion}
                  className="px-6 py-3 bg-gradient-to-r from-blue-500 to-purple-500 rounded-xl text-white font-medium transform transition hover:scale-105 glow-effect"
                >
                  Next Challenge
                </button>
              </div>
        ...
        {/* Quiz Configuration Modal with Close Options */}
        ...
        {/* Practice Configuration Modal with Close Options */}
        {showPracticeConfig && (
          <div 
            className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4"
        ...
        {/* Enhanced More Categories Modal with Colorful Icons */}
        {showMoreCategories && (
          <div 
            className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4"
        ...
        {/* Enhanced Coins Info Modal - Apple-like Design */}
        {showCoinsInfo && (
          <div 

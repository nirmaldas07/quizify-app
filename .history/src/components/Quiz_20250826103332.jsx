...
/* ---------------- XP & Coins (same as Home) ---------------- */
const XP_KEY = "qp_xp";
const LVL_KEY = "qp_level";
const COIN_KEY = "qp_coins";
const COIN_TO_XP = 2;                         // 2 XP per coin
const threshold = (n) => 100 * n * (n - 1);   // same leveling curve as Home
...
const [pendingReward, setPendingReward] = useState(null); // { startRect, coins, xp }
const [lastResult, setLastResult] = useState({ correct: 0, total: 0 });
const [showFlash, setShowFlash] = useState(false);
const [countCorrect, setCountCorrect] = useState(0);
const [countCoins, setCountCoins] = useState(0);
const [retakeRun, setRetakeRun] = useState(Boolean(location.state?.retake)); // true = this run is a retake
...
const awardCoins = useCallback((amount) => {
  ...
  if (!isReview) return;
   try {
     ...
     // feed the Review screen directly
     setReviewSnapshot({
  ...
  useEffect(() => {
    if (isReview) return;   // ← prevent fresh session while reviewing
    ...
    if (resumeFlag) {
      ...
      if (saved?.slug === category && saved?.inProgress) {
        ...
        setRetakeRun(Boolean(location.state?.retake));
        return;
      }
    }
    ...
    }, [loading, err, allQuestions, category, difficulty, count, resumeFlag, timerConfig?.seconds, timerConfig?.type, location.search, isReview]);
...
useEffect(() => {
  ...
  // If there is a pendingReward and it hasn't finished flying yet,
  // wait until onDone marks it as awarded.
  if (pendingReward && !pendingReward.awarded) return;

  const t = setTimeout(() => setView("results"), 2000);
  return () => clearTimeout(t);
}, [view, pendingReward]);
...
const onSubmitConfirmed = (e) => {
  ...
  // ❗ Retakes earn no coins/XP
  const noReward = isPractice || isRetake || retakeRun;
  const coinsEarned = noReward ? 0 : (correct * 5);
  const xpEarned    = coinsEarned * COIN_TO_XP;
  ...
  // Stage Reward view (2 seconds)
  setLastResult({ correct, total });
  if (coinsEarned > 0) {
    setPendingReward({ startRect, coins: coinsEarned, xp: xpEarned });
  } else {
    setPendingReward(null);
  }

  setView("reward");          // 👈 go to the transition screen
  setPaused(true);
  clearInterval(tickInterval.current);
  persistResume(false);
  clearResume();
  ...
  const retakeSameSet = () => {
    ...
    setRetakeRun(true);
    setView("quiz");
    // Preserve the fromHistory flag for retakes initiated from review
    if (location.state?.reviewFrom === "history") {
    // Update location state to maintain fromHistory context
...
{view === "reward" && pendingReward && pendingReward.coins > 0 && !pendingReward.awarded && (
  <CoinFly
    startRect={pendingReward.startRect}
    targetRef={coinPillRef}
    count={Math.min(28, Math.max(12, Math.round(pendingReward.coins / 2)))}
    onDone={() => {
      // award once, then mark as awarded (do not remove the object yet)
      awardXP(pendingReward.xp);
      awardCoins(pendingReward.coins);
      setPendingReward(prev => (prev ? { ...prev, awarded: true } : prev));
    }}
  />
)}
...
{view === "reward" && (
  <>
            ...
            <Modal onClose={() => setShowQuit(false)}>
              <div className="text-center">
                ...
                <div className="flex justify-center gap-3">
                  ...
                  <button className="btn-primary px-3 py-2 rounded-xl" onClick={() => {
    ...
    {view === "results" && (
        <>
            {/* results HUD with back button */}
            <div className="mt-3 flex items-center justify-between px-3">
            {location.state?.fromHistory && (
                <button
                onClick={() => navigate("/profile/history", { replace: true, state: null })}
                className="px-3 py-1.5 rounded-xl border border-white/20 bg-white/10 text-white text-sm flex items-center gap-1"
                >
                ← History
                </button>
            )}
            <div className={`flex items-center gap-2 ${!location.state?.fromHistory ? 'ml-auto' : ''}`}>
                <XPill level={level} xp={xp} next={nextThreshold} onClick={()=>setShowLevels(true)} />
                <CoinPill total={coins} onClick={()=>setShowCoins(true)} pillRef={coinPillRef} />
            </div>
            </div>
        ...
        <div className="flex gap-2 justify-center">
            ...
            onClick={() => { setReviewSnapshot({ questions: session.questions, answers }); setView("review"); }}
            >
            Review Answers
            </button>
            <button 
            className="flex-1 max-w-[140px] py-3 rounded-2xl border border-white/20 text-white font-medium text-sm hover:bg-white/10 transition-colors"
            onClick={retakeSameSet}
            >
            Retake Quiz
            </button>
        </div>
        <div className="flex gap-2 justify-center">
            <button 
            className="flex-1 max-w-[140px] py-2.5 rounded-xl border border-white/20 bg-white/5 text-white text-sm hover:bg-white/10 transition-colors"
            onClick={() => navigate("/profile/history", { replace: true, state: null })}
            >
            History
            </button>
...
<Sheet open={showLevels} onClose={()=>setShowLevels(false)}>
  <div className="relative">
    <button className="absolute right-0 -top-2 text-sm px-2 py-1 rounded-lg bg-white/5 border border-base-border" onClick={()=>setShowLevels(false)}>✕</button>
  </div>
  ...
  <div className="rounded-xl bg-white/5 border border-base-border p-3 mb-4">
    <div className="text-sm mb-1">Next level in <b>{Math.max(0, threshold(level+1) - xp)}</b> XP</div>
    <div className="text-xs text-base-muted">At {COIN_TO_XP} XP/coin, you'd need ~<b>{Math.ceil(Math.max(0, threshold(level+1) - xp)/COIN_TO_XP)}</b> coins.</div>
  </div>
...
<Sheet open={showCoins} onClose={()=>setShowCoins(false)}>
  <div className="relative">
    <button className="absolute right-0 -top-2 text-sm px-2 py-1 rounded-lg bg-white/5 border border-base-border" onClick={()=>setShowCoins(false)}>✕</button>
  </div>
  <h3 className="text-lg font-semibold mb-3">Earn coins</h3>
  <div className="space-y-2">
    <button className="w-full rounded-xl border border-base-border bg-white/5 px-3 py-3 text-left" onClick={() => { setShowCoins(false); navigate("/"); }}>
      <div className="font-medium">Attempt a quiz</div>
      <div className="text-xs text-base-muted">50 coins / quiz</div>
    </button>
    <button className="w-full rounded-xl border border-base-border bg-white/5 px-3 py-3 text-left" onClick={() => { setShowCoins(false); navigate("/play"); }}>
      <div className="font-medium">Spin & Play</div>
      <div className="text-xs text-base-muted">5 coins / spin</div>
    </button>
    <button className="w-full rounded-xl border border-base-border bg-white/5 px-3 py-3 text-left" onClick={() => alert("Video screen will come soon")}>
      <div className="font-medium">Watch a video</div>
      <div className="text-xs text-base-muted">20 coins / video</div>
    </button>
    <button className="w-full rounded-xl border border-base-border bg-white/5 px-3 py-3 text-left" onClick={() => {
...
/* ===== Pills & Sheet (copied from Home so it looks identical) ===== */

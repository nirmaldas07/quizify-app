...
function CoinPill({ total, onClick, pillRef }) {
  return <button ref={pillRef} onClick={onClick} className="px-3 py-1 rounded-2xl bg-white/5 border border-base-border">🪙 {total}</button>;
}
function WheelIcon({ size = 76 }) {
  return (
...
/* ------------ Modal Sheet (z-index over bottom nav) ------------ */
...
export default function Home() {
  ...
  /* Countdown to midnight for daily question */
  ...
  /* =========================== UI =========================== */
  return (
    <div className="relative min-h-screen bg-base-bg text-base-text pb-24 pt-[calc(env(safe-area-inset-top)+1rem)]">
      {/* ✅ CoinFly animation instance */}
      {coinFly && (
        <CoinFly
      ...
      {/* Header — Play-style: compact, bold, pills on right */}
      <header className="mx-auto max-w-md px-5 mb-5">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button ref={coinPillRef} onClick={()=>setShowCoins(true)} className="px-3 py-1 rounded-2xl bg-white/5 border border-base-border">🪙 {coins}</button>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-md px-5">
        ...
        <button
          ...
          className="group w-full rounded-3xl p-6 mb-6 text-left bg-gradient-to-r from-blue-400 to-blue-500 shadow-lg border border-white/20 relative overflow-hidden"
        >
          <div className="absolute top-0 right-0 w-40 h-40 opacity-25">
            <svg viewBox="0 0 100 100" className="w-full h-full animate-spin" style={{ animationDuration: '18s' }}>
              {[0,1,2,3,4,5].map(i=>{
                const colors=["#7C3AED","#10B981","#EF4444","#F59E0B","#2563EB","#FACC15"];
                const slice=60; const start=i*slice; const end=start+slice;
                const sr=(Math.PI*start)/180, er=(Math.PI*end)/180;
                const x1=50+40*Math.cos(sr), y1=50-40*Math.sin(sr);
                const x2=50+40*Math.cos(er), y2=50-40*Math.sin(er);
                return <path key={i} d={`M50,50 L${x1},${y1} A40,40 0 0 0 ${x2},${y2} Z`} fill={colors[i]} stroke="white" strokeWidth="0.4"/>;
              })}
          ...
          <div className="flex items-center justify-between relative z-10">
            <div>
              <div className="text-[11px] uppercase tracking-wider text-white/90">Quick Play</div>
              <div className="text-3xl font-black text-white leading-tight">SPIN & PLAY</div>
              <div className="text-sm text-white/90 mt-1">1 quick question • Earn XP & coins</div>
            </div>
            <WheelIcon size={72} />
          </div>
        </button>
        ...
        {resume?.inProgress && (
          <div className="rounded-2xl border border-base-border bg-white/5 p-3 mb-5 flex items-center justify-between">
        ...
        {/* Streak Center — slimmer Play look */}
        <section className="rounded-3xl border border-white/15 bg-gradient-to-br from-indigo-500/15 to-fuchsia-500/10 p-4 mb-6">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="text-sm text-base-muted">
                🔥 {streak} days <span className="mx-1 text-base-muted/60">|</span> Best {bestStreak} <span className="mx-1 text-base-muted/60">|</span> Keep going!
              </div>
              <div className="mt-2">
        ...
        {/* Today’s Question — Play card style with flip */}
        {dailyQ && (
          <section className="rounded-3xl border border-white/15 bg-gradient-to-br from-blue-500/15 to-cyan-500/10 p-0 mb-8 overflow-hidden">
            <div className="px-5 pt-4">
              ...
              <div className="text-xs text-base-muted mb-3">Category: {dailyQ.category}</div>
            </div>
        ...
        <div ref={categoriesRef} className="grid grid-cols-3 gap-4 mb-6">
          {categories.map((cat) => (
        ...
        {/* Daily Challenge — Play gradient */}
        <div className="rounded-3xl border border-white/15 p-4 mb-8 bg-gradient-to-br from-blue-500/20 to-purple-500/20">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-sm text-base-muted">Daily Challenge</div>
              <div className="text-sm mt-1">
                <div><span className="text-base-muted">Category:</span> {readJSON(LS_LAST_DAILY, defaultDaily)?.category || "All"}</div>
                <div><span className="text-base-muted">Difficulty:</span> {cap(readJSON(LS_LAST_DAILY, defaultDaily)?.difficulty || "medium")}</div>
                <div><span className="text-base-muted">Question count:</span> {readJSON(LS_LAST_DAILY, defaultDaily)?.count || 10}</div>
              </div>
            </div>
            <div className="flex items-center gap-2">
        ...
        {/* Learn — Practice (Play gradient) */}
        <h2 className="text-lg font-semibold mb-2">Learn</h2>
        <div className="rounded-3xl border border-white/15 p-4 mb-4 bg-gradient-to-br from-fuchsia-500/20 to-blue-500/15">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-sm text-base-muted">Practice Questions</div>
              <div className="text-sm mt-1">
                <div><span className="text-base-muted">Category:</span> { (readJSON(LS_LAST_PRACT, defaultPractice)?.category) || "General Knowledge" }</div>
                <div><span className="text-base-muted">Difficulty:</span> {cap(readJSON(LS_LAST_PRACT, defaultPractice)?.difficulty || "medium")}</div>
                <div><span className="text-base-muted">Question count:</span> {readJSON(LS_LAST_PRACT, defaultPractice)?.count || 10}</div>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                onClick={()=>{
                  const last = readJSON(LS_LAST_PRACT, defaultPractice);
                  const cat = last?.category || "General Knowledge";
                  const diff = last?.difficulty || "medium";
                  const count = last?.count || 10;
                  writeJSON(LS_LAST_PRACT, { difficulty: diff, count });
                  navigate(`/quiz/${toSlug(cat)}/start`, {
                    state: { mode: "practice", difficulty: diff, count, timer: { type: "off", seconds: 0 } }
                  });
                }}
              >
                Start
        ...
        {/* Fun Facts lane (unchanged layout, tightened spacing) */}
        <div className="flex items-center justify-between mb-2">

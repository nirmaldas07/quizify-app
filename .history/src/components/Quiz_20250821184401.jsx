...
const onSelect = (optIdx, evt) => {
  ...
  /* -------- Lifelines -------- */
  const use5050 = () => {
    if (used5050) { setToast("Once per quiz"); return; }
    if (!current) return;
  ...
  const useAudience = () => {
    if (usedAudience) { setToast("Once per quiz"); return; }
    if (!current) return;
...
<button
  ...
  className={["px-3 py-2 rounded-xl border border-base-border bg-white/5",
             (used5050 || readOnly) ? "opacity-60" : ""].join(" ")}
  title={used5050 ? "Once per quiz" : "50:50 (eliminate two options)"}  // 👈
...
<button
  ...
  disabled={readOnly}
  className={["px-3 py-2 rounded-xl border border-base-border bg-white/5",
             (usedAudience || readOnly) ? "opacity-60" : ""].join(" ")}
  title={usedAudience ? "Once per quiz" : "Ask the audience"}  // 👈

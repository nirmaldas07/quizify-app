  ...
  useEffect(() => {
    const handleNavClick = (e) => {
      if (isInQuiz && !showResults) {
        const navButton = e.target.closest('[aria-current]');
        const swipeText = e.target.closest('.text-\\[10px\\]')?.textContent;
        
        if ((navButton && navButton.textContent.includes('Swipe')) || swipeText === 'Swipe') {
          e.preventDefault();
          e.stopPropagation();

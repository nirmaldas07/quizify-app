  ...
  return (
    <Screen title="Leaderboard" subtitle="Ends in 2d : 14h">
      ...
      <ul className="space-y-2">
        {data.map((u) => (
          <li key={u.rank} className={`rounded-2xl border border-white/10 p-3 flex items-center justify-between ${
            u.you ? "bg-white/10" : "bg-white/5"}
          }>
            <div className="flex items-center gap-3">
  ...
  return (
    <Screen title="Badges" subtitle="Collect and show off">
      <div className="grid grid-cols-4 gap-2">
        {mockBadges.map((b) => (
          <div key={b.id} className={`aspect-square rounded-2xl grid place-items-center border ${
            b.unlocked ? "bg-white/5 border-white/10" : "bg-transparent border-white/10"}
          }>

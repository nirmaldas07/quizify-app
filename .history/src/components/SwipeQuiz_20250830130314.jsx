...
const SwipeQuiz = () => {
 ...
 // Lifelines tracking
 const [lifeline5050Used, setLifeline5050Used] = useState(-1);
 const [lifelineAudienceUsed, setLifelineAudienceUsed] = useState(-1);
 ...
 // Hide bottom nav when in quiz
 useEffect(() => {
   if (!showDiscovery && !showResults) {
 ...
 // Improved scroll handling with single question snap
 useEffect(() => {
   ...
   const handleTouchEnd = (e) => {
     ...
     if (Math.abs(diff) > threshold) {
       // Haptic feedback for manual swipe
       ...
       if (diff > 0 && currentIndex < questions.length - 1) {
         ...
         setCurrentIndex(prev => prev - 1);
         const prevElement = document.getElementById(`question-${currentIndex - 1}`);
 ...
 const handleFiftyFifty = (questionId) => {
   ...
   // Check if can use
   if (lifeline5050Used >= 0) {
     const attemptedSinceLast = Array.from(attemptedQuestions).filter(
       idx => idx > lifeline5050Used && idx < questionIndex
     ).length;
     
     if (attemptedSinceLast < 5) {
       setFeedbackMessage(`After ${5 - attemptedSinceLast} attempts`);
   ...
   setLifeline5050Used(questionIndex);
 };
 ...
 const handleAudience = (questionId) => {
   ...
   // Check if can use
   if (lifelineAudienceUsed >= 0) {
     const attemptedSinceLast = Array.from(attemptedQuestions).filter(
       idx => idx > lifelineAudienceUsed && idx < questionIndex
     ).length;
   ...
   setLifelineAudienceUsed(questionIndex);
 };
 ...
 const handleCategorySelect = (category) => {
   ...
   setSelectedCategoryName(categoryNames[category] || category);
 ...
 const ResultsScreen = () => {
   ...
   useEffect(() => {
     const timer = setTimeout(() => {
       ...
       // Navigate to Swipe home instead of general home
       navigate('/swipe');
     }, 2000);
   ...
   <div className="h-screen bg-gray-900 relative">
           ...
           {Array.from({ length: 5 }, (_, i) => (
             <span 
               ...
               className={`${i < lives ? 'text-red-500' : 'text-gray-600 opacity-30'} ${
                 heartAnimation && i === lives - 1 ? 'animate-heartBreak' : ''
               }`}
             >
               ❤️
             </span>
     ...
     {coinAnimation && (
       <div className="fixed top-20 left-1/2 transform -translate-x-1/2 z-50 pointer-events-none">
         <div className="animate-coinFly">
           <span className="text-3xl">🪙</span>
           <span className="text-yellow-400 font-bold ml-1">+10</span>
         </div>
       </div>
     )}
     ...
     <div 
       ref={containerRef}
       className="h-full overflow-y-scroll snap-y snap-mandatory pt-32 scroll-smooth"
       style={{ 
     ...
     >
       {questions.map((question, index) => (
         <QuestionSlide
           ...
           canUseFiftyFifty={
             lifeline5050Used < 0 || 
             Array.from(attemptedQuestions).filter(idx => idx > lifeline5050Used && idx < index).length >= 5
           }
           canUseAudience={
             lifelineAudienceUsed < 0 || 
             Array.from(attemptedQuestions).filter(idx => idx > lifelineAudienceUsed && idx < index).length >= 5
           }
         />
       ))}
     </div>

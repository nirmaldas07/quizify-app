...
// Check for stored results when coming back from history
useEffect(() => {
  if (location.state?.returnToResults) {
    const storedResults = GameDataService.getCurrentResults();
    if (storedResults) {
      setReviewSnapshot(storedResults);
      setView("results");
      setLoading(false); // Prevent other useEffects from running
      setSession({ questions: [] }); // Clear session to prevent quiz from starting
    }
  }
}, [location.state]);
...
useEffect(() => {
  ...
  // Skip if already showing results - REMOVE THIS CHECK
  ...
  if (allQuestions.length === 0) return;
    ...
    // Reset view to quiz when navigating to new quiz
    setView("quiz");
    setReviewSnapshot(null);
  ...
  }, [allQuestions, category, mode, difficulty, count, location.pathname]); // Changed dependencies
...
// Handle external review - UNCOMMENT AND FIX THIS
useEffect(() => {
  if (!isReview) return;
  
  console.log("Review mode detected from URL");
  
  try {
    const raw = localStorage.getItem(REVIEW_KEY);
    if (!raw) {
      console.error("No review snapshot in localStorage");
      return;
    }
    
    const snap = JSON.parse(raw);
    console.log("Loaded review snapshot:", snap);
    ...
    setReviewSnapshot({
      ...
      mode: snap?.mode || "quiz",
      isPractice: snap?.isPractice || false
    });
...
if (currentUser2 && currentUser2.phone) {
  ...
  const practiceResult = GameDataService.recordPracticeSession(currentUser2.phone, {
  questionsCompleted: results.correct,  // Only count correct as "completed"
...
// Store in GameDataService
GameDataService.storeCurrentResults(resultsData);
...
if (error) {
  ...
  return (
  <div className="fixed inset-0 bg-inherit text-white z-0 overflow-hidden max-w-[100vw]">

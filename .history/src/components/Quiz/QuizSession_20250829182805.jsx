...
if (isCorrect) {
  ...
  if (isPractice) {
    ...
    if (index === 0 && answerPattern.length === 0) {
      feedback = { emoji: "🎯", message: "Perfect start! You're a natural!" };
    }
    // Streak messages
    else if (consecutiveCorrect === 2) {
      feedback = { emoji: "🔥", message: "2 in a row! Keep it up!" };
    }
    else if (consecutiveCorrect === 3) {
      feedback = { emoji: "⚡", message: "3 streak! You're on fire!" };
    }
    else if (consecutiveCorrect === 5) {
      feedback = { emoji: "🚀", message: "5 streak! Unstoppable!" };
    }
    else if (consecutiveCorrect === 10) {
      feedback = { emoji: "👑", message: "10 streak! Legendary!" };
    }
    // Recovery after wrong
    else if (answerPattern[answerPattern.length - 1] === false) {
      feedback = { emoji: "💪", message: "Great recovery! Back on track!" };
    }
    // Milestone messages
    else if (index === Math.floor(total * 0.25) - 1) {
      feedback = { emoji: "🎬", message: "25% done! Looking good!" };
    }
    else if (index === Math.floor(total * 0.5) - 1) {
      feedback = { emoji: "⭐", message: "Halfway there! Keep going!" };
    }
    else if (index === Math.floor(total * 0.75) - 1) {
      feedback = { emoji: "🏁", message: "Almost done! Final push!" };
    }
    // Generic correct
    else {
      const messages = [
...
{isPractice && feedbackMessage && (
  <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50 pointer-events-none animate-fadeInOut">
    <div className="bg-gradient-to-r from-purple-600 to-blue-600 text-white px-8 py-4 rounded-2xl shadow-2xl flex items-center gap-3 border-2 border-white/20">
      <span className="text-3xl">{feedbackMessage.emoji}</span>
      <span className="font-bold text-xl">{feedbackMessage.message}</span>
    </div>
  </div>
)}
      ...
      {/* Main Content - Always scrollable with fixed bottom nav */}
      <div className="flex-1 px-4 flex flex-col overflow-y-auto pb-36">
        {/* Question Card */}

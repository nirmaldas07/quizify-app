...
const [showButtons, setShowButtons] = useState(true);
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Hide when scrolling down (but only after a delay to avoid flicker)
      ...
      // Always show at the very top or bottom
      if (currentScrollY <= 50 || window.innerHeight + currentScrollY >= document.body.scrollHeight - 50) {
        setShowButtons(true);
      }
  ...
  return (
    ...
    {/* Action Buttons */}
      <div 
        className={`fixed bottom-2 left-4 right-4 flex gap-2 justify-center transition-transform duration-300 ${
          showButtons ? 'translate-y-0' : 'translate-y-24'
        }`}
      >
        <button
          className="flex-1 max-w-[150px] px-4 py-3 rounded-xl border border-base-border bg-base-card backdrop-blur shadow-lg"
          onClick={handleBack}
        >
          Back to Results
        </button>
        {onRetake && (
          <button 
            className="flex-1 max-w-[150px] px-4 py-3 rounded-xl bg-gradient-to-r from-brand-blue to-brand-purple text-white font-medium shadow-lg"
            onClick={onRetake}
          >
            Retake Quiz
          </button>

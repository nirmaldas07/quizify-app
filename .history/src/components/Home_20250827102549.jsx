...
export default function Home() {
  ...
  const { player, addXP, addCoins, levelProgress, useEnergy } = useGame();
  ...
  useEffect(() => {
    if (player.level > prevLevelRef.current) {
      playLevelUp?.();
      // Show level up animation/modal here
  ...
  const handleCategorySelect = (category) => {
    ...
    if (player.energy < GAME_CONSTANTS.ENERGY_COST.quiz) {
      // Show no energy modal
      alert("Not enough energy! Wait for regeneration or play Practice mode.");
      return;
    }
  ...
  const startQuiz = () => {
    if (useEnergy(GAME_CONSTANTS.ENERGY_COST.quiz)) {
  ...
  return (
    <div className="min-h-screen bg-base-bg text-base-text">
      ...
      <div className="relative z-10 px-4 pt-10 pb-24">
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-3 gap-3">
            {categories.map((cat) => (
              ...
              >
                {player.energy < 1 && cat.slug !== "more" && (
                  <div className="absolute inset-0 bg-black/50 rounded-2xl flex items-center justify-center">
                    <span className="text-xs">No Energy</span>
                  </div>
                )}
        ...
        {/* Keep all existing modals as they are, just add energy check in Quiz Config */}
        ...
        {/* Keep all other modals unchanged - Practice Config, More Categories, Coins Info */}

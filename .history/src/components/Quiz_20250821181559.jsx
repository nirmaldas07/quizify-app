    ...
    <ScreenWrap>
      {view === "quiz" && (
        ...
        <div className="flex flex-col items-center">
            <button
            onClick={() => { if (!used5050) use5050(); }}
            disabled={readOnly || used5050}
            className="px-3 py-2 rounded-xl border border-base-border bg-white/5 disabled:opacity-50"
            ...
            {used5050 && (
            <div className="text-xs text-red-400 mt-1">One per quiz</div>
            )}
        </div>

        <div className="flex flex-col items-center">
            <button
            onClick={() => { if (!usedAudience) useAudience(); }}
            disabled={readOnly || usedAudience}
            className="px-3 py-2 rounded-xl border border-base-border bg-white/5 disabled:opacity-50"

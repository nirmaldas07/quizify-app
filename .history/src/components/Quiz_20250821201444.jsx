...
const awardCoins = useCallback((amount) => {
  ...
  }, [loading, err, allQuestions, category, difficulty, count, resumeFlag, timerConfig?.seconds, timerConfig?.type]);
  ...
  // NEW — play coin sound when results open and we have a pending reward
    useEffect(() => {
    if (view === "results" && pendingReward) {
        try { playCoin(); } catch {}
    }
...
const onSelect = (optIdx, evt) => {
  ...
  const onTimeUp = () => {
    ...
    const coinsEarned = correct * 5;               // 5 coins per correct
    const xpEarned    = coinsEarned * COIN_TO_XP;  // 2 XP per coin
    ...
    // stage coin animation to run on Results

...
const SwipeQuiz = () => {
  ...
  const loadQuestions = async (category) => {
    try {
      ...
      if (category === 'level') {
        ...
        filtered = parsedQuestions.filter(q => q.difficulty === 'medium');
      } else if (category === 'daily') {
        // Daily challenge - mix of all
        filtered = parsedQuestions.slice(0, 20);
      }
  ...
  const handleFiftyFifty = (questionId) => {
    ...
    // Check if can use (5 questions cooldown)
    if (used5050 > 0 && questionIndex < used5050 + 5) {
      showFeedback(`Try after ${used5050 + 5 - questionIndex} questions`);
      return;
    }
  ...
  const handleAudience = (questionId) => {
    ...
    if (usedAudience > 0 && questionIndex < usedAudience + 5) {
      showFeedback(`Try after ${usedAudience + 5 - questionIndex} questions`);
      return;
    }
  ...
  const handleAnswer = (questionId, optionIndex) => {
    ...
    // Reset mascot after 2s
    setTimeout(() => setMascotReaction('neutral'), 2000);
  ...
  return (
    <div className="h-screen bg-gray-900 relative">
          ...
          {['🐼', '🐱', '🐶', '🦊', '🐰'].map((emoji, i) => (
            <span 
              ...
              className={`text-2xl transition-all ${
                mascotReaction === 'happy' ? 'animate-bounce' :
                mascotReaction === 'sad' ? 'animate-pulse opacity-50' :
                'animate-pulse'
              }`}
      ...
      <div 
        ref={containerRef}
        className="h-full overflow-y-scroll snap-y snap-mandatory pt-28"

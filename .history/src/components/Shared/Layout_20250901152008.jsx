...
const HIDE_ROUTES = [
  ...
  const handleTouchStart = (e) => {
    ...
    if (window.scrollY === 0) {
      ...
      touchStartY.current = null; // Reset if not at top
    }
  };
  ...
  const handleTouchMove = (e) => {
    ...
    // Only process if we started the touch at the top
    if (touchStartY.current === null || touchStartY.current === undefined) return;
    
    // Only allow pulling if still at top
    if (window.scrollY === 0) {
      ...
      if (dist > 0) {
        ...
        setPullProgress(Math.min((cappedDist / 50) * 100, 100));
      ...
      // If we've scrolled away from top, cancel the pull
      setPulling(false);
      setPullDist(0);

...
const HIDE_ROUTES = [
  ...
  const handleTouchMove = (e) => {
    if (!inBottomNavPage || refreshing || touchStartY.current === null) return;
    if (window.scrollY === 0) {
      const dist = e.touches[0].clientY - touchStartY.current;
      if (dist > 0) {
        setPulling(true);
        const cappedDist = Math.min(dist, 100); // Reduced from 120
        setPullDist(cappedDist);
        setPullProgress(Math.min((cappedDist / 50) * 100, 100)); // Reduced threshold from 60 to 50
        
        // Light haptic at threshold
        if (dist > 50 && dist < 55) { // Reduced from 60-65
          haptic('light');
        }
      }
    }

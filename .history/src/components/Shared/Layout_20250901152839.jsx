...
useEffect(() => {
  ...
  const inBottomNavPage = pathname === "/";
...
const HIDE_ROUTES = [
  ...
  const handleTouchStart = (e) => {
    ...
    // Only register touch start if we're already at the top
    if (window.scrollY <= 1) { // Allow 1px tolerance
      touchStartY.current = e.touches[0].clientY;
    } else {
      // Explicitly set to null if not at top
  ...
  const handleTouchMove = (e) => {
    ...
    // Bail out early if we didn't start at the top
    if (touchStartY.current === null || touchStartY.current === undefined) {
      setPulling(false);
      setPullDist(0);
      setPullProgress(0);
      return;
    ...
    if (window.scrollY <= 1) {
      ...
      // Only register downward pulls with minimum threshold
      if (dist > 10) { // 10px threshold to prevent accidental triggers
        setPulling(true);
        const cappedDist = Math.min(dist - 10, 90); // Subtract threshold from distance
        setPullDist(cappedDist);
        setPullProgress(Math.min((cappedDist / 40) * 100, 100)); // Adjusted for new scale
        
        if (cappedDist > 40 && cappedDist < 45) {
          haptic('light');
        }
      } else {
        // Not enough pull distance
        setPulling(false);
        setPullDist(0);
        setPullProgress(0);
      }
    } else {
      // We've scrolled away from top - immediately cancel everything
      setPulling(false);
      setPullDist(0);
      setPullProgress(0);
      touchStartY.current = null;
  ...
  const handleTouchEnd = () => {
    // Only process if we're on home page and started at top
    if (!inBottomNavPage || touchStartY.current === null) {
      setPulling(false);
      setPullDist(0);
      setPullProgress(0);
      touchStartY.current = null;
    ...
    // Only trigger refresh if we have sufficient pull distance
    if (pulling && pullDist > 40 && window.scrollY <= 1) {
      setRefreshing(true);
      playSound('refresh');
      haptic('success');
  ...
  return (
    ...
    {(pulling || refreshing) && pathname === "/" && (
      <div
        className="fixed left-1/2 z-50 flex flex-col items-center transition-all duration-300"
        style={{
          top: refreshing ? "40px" : `${Math.max(10, Math.min(pullDist - 20, 60))}px`,
          transform: "translateX(-50%) scale(" + (refreshing ? 1 : 0.9) + ")",
        }}
      >
        {/* Progress circle - Reduced size */}
        <div className="relative mb-1">
          <svg width="32" height="32" className={refreshing ? "animate-spin" : ""}>
            <circle
              cx="16"
              cy="16"
              r="14"
            ...
            <circle
              ...
              stroke="url(#gradient)"
              strokeWidth="3"
              strokeDasharray={`${pullProgress * 0.88} 88`}
              strokeLinecap="round"
        ...
        <span className={`text-xs font-medium text-white px-2 py-0.5 rounded-full bg-white/20 backdrop-blur
          ${refreshing ? 'rainbow-text' : ''}`}>
          {refreshing ? "Done!" : pullDist > 50 ? "Release" : "Pull"}
        </span>

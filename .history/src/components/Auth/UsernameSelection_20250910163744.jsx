...
import './auth.css';
import { saveUserToFirestore } from '../../utils/firestore.js';
export default function UsernameSelection() {
  const location = useLocation();
  const navigate = useNavigate();
  const { phone, avatar, firebaseUser } = location.state || {};
  ...
  const handleComplete = () => {
    ...
    // Generate a default password for new signup (you should implement proper password flow)
    const defaultPassword = password || '123456'; // This should be collected from user in a real app

    // Save user data to users database
...
// Also save to Firestore (only on mobile)
if (typeof window !== 'undefined' && window.Capacitor?.platform && window.Capacitor.platform !== 'web') {
  saveUserToFirestore(phone, {
    username,
    gender,
    ageGroup,
    avatar,
  ...
  console.log('Firestore skipped - running in browser');
}



    // Store Firebase user if available
    if (firebaseUser) {
    ...
    // Save to currentUser for session
    ...
    localStorage.setItem('qp_player', JSON.stringify(gamePlayer));
  ...
  const handleBack = () => {
    ...
    state: { phone, avatar, firebaseUser } 
    });
  };

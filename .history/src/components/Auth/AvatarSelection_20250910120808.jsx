...
export default function AvatarSelection() {
  ...
  const { phone, firebaseUser } = location.state || {};
  ...
  const handleContinue = () => {
    if (selectedAvatar) {
      navigate('/auth/username', { 
        state: { phone, avatar: selectedAvatar, firebaseUser } 
     });
    }
  };
  ...
  return (
    <div className="auth-screen" style={{
      ...
      <button 
        ...
        onClick={() => navigate('/auth', { state: { phone: phone?.replace(/^\+\d{1,3}/, ''), fromAvatar: true } })}
        style={{
          position: 'absolute',
          top: '40px',
          left: '20px',

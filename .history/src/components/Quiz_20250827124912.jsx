...
// }
    ...
    isReview,
    isPractice,
// Remove these from dependencies temporarily
    // energy,
    // useEnergy,
    navigate
  ]);
  ...
  // Handlers
    const handleQuizComplete = (results) => {
        const safe = normalizeResults(results, session, {
            elapsedMs,
            categorySlug: category,
            mode,
            timerConfig,
             });
             // Save mistakes (guarded)
            const wrongIds = (safe.questions || [])
            .filter((q, i) => {
            const a = safe.answers?.[i];
            return a !== null && a !== undefined && a !== q.answerIndex;
          })
           .map(q => q.id);
    try {
    ...
    try {
      ...
      stats.sessions.push({
        ...
        skipped: Array.isArray(safe.skipped) ? safe.skipped.filter(Boolean).length : 0,
        ms: elapsedMs
      });

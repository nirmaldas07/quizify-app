...
export default function Quiz() {
  ...
  const { energy, useEnergy, player, addCoins, addXP } = useGame();
...
// Calculate coins but DON'T add them yet - let QuizResults handle it after animation
let coinsToAdd = 0;
if (isPractice && results.earnedCoins) {
  // For practice mode, use the earned coins from the session
  coinsToAdd = results.earnedCoins;
} else if (!isPractice && results.correct > 0) {
  // For quiz mode, calculate based on correct answers
...
setReviewSnapshot({
  ...
  hasTimer: !isPractice,
  earnedXP: isPractice ? 0 : results.correct * 10,
  earnedCoins: isPractice ? (results.earnedCoins || 0) : results.correct * 5  // Use earnedCoins from practice session
});
    setView("results");
...
{view === "results" && reviewSnapshot && (
  <QuizResults
    ...
    addCoins={addCoins}  // Add this line

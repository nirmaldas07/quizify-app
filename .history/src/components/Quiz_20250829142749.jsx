...
// Add coins and XP to player (only for quiz mode, not practice)
if (!isPractice && results.correct > 0) {
  const coinsToAdd = results.correct * 5;
  const xpToAdd = results.correct * 10;
  console.log(`Adding ${coinsToAdd} coins and ${xpToAdd} XP`);
  addCoins(coinsToAdd);
  addXP(xpToAdd);
}
...
setReviewSnapshot({
      ...
      categorySlug: category,  // ADD THIS LINE

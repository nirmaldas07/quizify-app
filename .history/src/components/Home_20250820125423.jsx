...
export default function Home() {
  ...
  const startDailyQuick = () => {
    ...
    navigate(`/quiz/${last?.category === "All" ? "mixed" : toSlug(last?.category || "mixed")}`, {
  ...
  const goResume = () => {
    ...
    if (!r?.slug) return;
    navigate(`/quiz/${r.slug}`, { state: { resume: true } });
  };
          ...
          <div key={c.id} className="min-w-[240px] snap-start rounded-2xl bg-white/5 border border-base-border p-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                onClick={() => navigate(`/quiz/${toSlug("fun-facts")}`, { state: { mode:"quiz", count:3, difficulty:"easy", timer:{ type:"per_q", seconds:20 }, source:"fun_facts" } })}
              >
                ➡️ Mini-Quiz (3)
              </button>
            </div>
      ...
      <Sheet open={showDailyConfig} onClose={()=>setShowDailyConfig(false)}>
        ...
        <button
          ...
          onClick={()=>{
            ...
            navigate(`/quiz/${dailyCategory === "All" ? "mixed" : toSlug(dailyCategory)}`, {

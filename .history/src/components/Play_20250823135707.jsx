...
useEffect(() => {
  // re-arm the width animation whenever we come back to the wheel for the next question
  setArmFill(false);
...
<div className="flex justify-center mb-6">
  <div className="w-80 max-w-full">
    {/* keyframes once per page (safe to inline) */}
    <style>{`
...
>
      {[0,1,2].map((i) => {
        ...
        return (
          <div key={i} className="relative h-full bg-white/10">
            {/* static fill (instantly shows attempted) */}
            <div
      ...
      style={{
        ...
        transition: i === pendingProgressIdx ? "width 800ms ease-out" : "none"
            ...
            {/* sweep line to imply motion when we just returned to wheel */}
            {pulseIdx === i && (
...
export default function Play() {
  ...
  useEffect(() => {
  try { localStorage.setItem('qp_in_question', JSON.stringify(gameState === 'question')); } catch {}
  return () => { try { localStorage.removeItem('qp_in_question'); } catch {} };
}, [gameState]);
  ...
  // animate only the newly-completed segment
  ...
  const t2 = setTimeout(() => {
    if (postWheelNext) {
      if (postWheelNext === "characterPicker") {
...
}, [gameState, pendingProgressIdx, postWheelNext, soundOn]);

  
useEffect(() => {
  // Only animate when we're on the wheel and we actually have a segment to show
  if (gameState !== 'wheel' || pendingProgressIdx == null) return;

  // Play the progress sound and sparkle only for the newly completed segment
  setShowSparkle(pendingProgressIdx);
  if (soundOn) playSound('/sounds/progress.mp3', 0.6);

  const t1 = setTimeout(() => setShowSparkle(-1), 900);
  const t2 = setTimeout(() => {
    // After the short animation, go wherever we planned (after Q3)
    if (postWheelNext) {
      if (postWheelNext === 'characterPicker') {
        setGameState('characterPicker');
      } else {
...
const handleAnswer = (correct, motivator, isContinue = false) => {
  // 1) First click = user selected an option → just play the right/wrong sound and return.
  if (!isContinue) {
    if (soundOn) playSound(correct ? "/sounds/correct.mp3" : "/sounds/wrong.mp3", 0.7);
    return;
  }

  // 2) "Continue" clicked on the result overlay — now lock in progress and move back to the Wheel.
  const currentIdx = run.qIndex;              // capture BEFORE incrementing
  const nextProgress = [...run.progress];
  nextProgress[currentIdx] = true;            // mark this step as attempted (correct or not)
  ...
  // 3) Tell the Wheel which single segment to animate when we land there
  setPendingProgressIdx(currentIdx);

  // 4) Decide what happens AFTER the short progress animation on the Wheel
  if (currentIdx === 2) {
    const perfect = nextProgress.every(Boolean);
    // after Q3: show the progress on Wheel first, then go to character picker (if perfect) or back to modes
    setPostWheelNext(perfect ? "characterPicker" : "modes");
  } else {
    // after Q1/Q2: stay on Wheel for the next spin
    setPostWheelNext(null);
  }

  // 5) Go back to the Wheel now; the wheel-side useEffect will play the sound/sparkle and then route if needed.
  setGameState("wheel");
};

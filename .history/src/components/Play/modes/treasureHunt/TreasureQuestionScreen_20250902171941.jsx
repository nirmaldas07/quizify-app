...
export default function TreasureQuestionScreen({
  ...
  useEffect(() => {
    ...
    useEffect(() => {
        const handlePopState = (e) => {
            e.stopImmediatePropagation(); // Stop event from bubbling to parent
            if (!showQuitModal) {
                setShowQuitModal(true);
                // Push state again for modal
                window.history.pushState({ questionScreen: true }, '', window.location.pathname);
            }
        };
        
        window.history.pushState({ questionScreen: true }, '', window.location.pathname);
        window.addEventListener('popstate', handlePopState, true); // Add true for capture phase
  ...
  const getTimeWarning = () => {
    ...
    setTimeout(() => {
        ...
        // Play sounds immediately with visual feedback
        if (soundOn) {
        if (correct) {
            // Play correct/digging sound
        ...
        if (gemFound) {
            ...
            setFoundGem(true);
            if (soundOn) {
                const gemAudio = new Audio('/sounds/gem.mp3');
                gemAudio.volume = 0.6;
                gemAudio.play().catch(() => {});
            }
            }, 800);
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br ${island.theme.background} relative overflow-hidden`}>
      ...
      <div className="px-3 pt-8 slide-in">
        <div 
          ref={questionRef}

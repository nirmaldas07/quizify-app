...
export default function TreasureRewardScreen({
  ...
  return (
    <div className={`fixed inset-0 bg-gradient-to-br ${island.theme.background} overflow-hidden flex flex-col`}>
      ...
      <div className="relative z-10 flex flex-col h-full px-4 pt-[calc(env(safe-area-inset-top)+2.5rem)] pb-[calc(env(safe-area-inset-bottom)+1rem)]">
        {/* Title - Compact */}
        <div className="text-center mb-3 slide-up">
          <h1 className="text-2xl font-black text-white drop-shadow-lg mb-1">
            {earnedTreasure ? 'TREASURE FOUND!' : 'ISLAND COMPLETE!'}
          </h1>
          <div className="text-4xl bounce-animation">
            {earnedTreasure ? island.rewards.treasurePiece : '🏝️'}
          </div>
        </div>

        {/* Results Card - Optimized */}
        <div className="glass-card-dark rounded-2xl p-3 max-w-sm w-full mx-auto mb-2 slide-up" style={{ animationDelay: '0.2s' }}>
          {/* Score */}
          ...
          {showTreasure && earnedTreasure && (
            <div className="text-center mb-2">
              ...
              <div className="text-white font-bold text-sm drop-shadow-lg">Treasure Piece Earned!</div>
            </div>
          )}

          {/* Coins & Gems - Compact with dark background */}
          <div className="grid grid-cols-2 gap-2 mb-2">
            <div className="bg-black/50 rounded-xl p-2 text-center border border-white/20">
              <div className="text-xl">🪙</div>
              <div className="text-xl font-black text-yellow-400 drop-shadow-lg">{animateCoins}</div>
              <div className="text-white/90 text-[10px] font-medium">Coins Earned</div>
            </div>
            <div className="bg-black/50 rounded-xl p-2 text-center border border-white/20">
              <div className="text-xl">💎</div>
              <div className="text-xl font-black text-cyan-400 drop-shadow-lg">{session.gemsFound}</div>
              <div className="text-white/90 text-[10px] font-medium">Gems Found</div>
            </div>
          </div>

          {/* Motivational Message */}
          <div className="text-center p-2 bg-black/30 rounded-xl">
            <p className="text-white/95 font-medium text-xs drop-shadow-md">{motivationalMessage}</p>
          </div>

          {/* Perfect Score Badge */}
          {perfectScore && (
            <div className="mt-2 text-center">
              <div className="inline-block px-3 py-1 bg-gradient-to-r from-yellow-400 to-amber-500 rounded-full shadow-lg">
        ...
        <div className="space-y-2 w-full max-w-sm mx-auto mt-auto slide-up" style={{ animationDelay: '0.4s' }}>
          {!earnedTreasure && (
            <button
              onClick={onRetry}
              className="w-full py-3 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-xl font-black text-base shadow-xl hover:shadow-2xl transition-all hover:scale-[1.02] active:scale-[0.98]"
            >
          ...
          <button
            onClick={onContinue}
            className="w-full py-3 bg-black/50 backdrop-blur text-white rounded-xl font-black text-base shadow-xl hover:shadow-2xl transition-all hover:scale-[1.02] active:scale-[0.98] border border-white/30"
          >
            <div className="flex items-center justify-center gap-2">

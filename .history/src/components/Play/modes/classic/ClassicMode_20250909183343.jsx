...
export default function ClassicMode() {
  ...
  const [bonusAnswerCorrect, setBonusAnswerCorrect] = useState(false);
...
if (correct) {
  ...
  setTimeout(() => {
    ...
    // Prepare stats for summary (for non-perfect rounds)
    if (!perfect) {
        const stats = {
        correctAnswers: updatedResults.filter(Boolean).length,
        totalQuestions: 3,
        coinsEarned: totalCoinsEarned,
        xpEarned: updatedResults.filter(Boolean).length,
        perfectRound: false,
        characterUnlocked: null,
        bonusCorrect: false
        };
        setGameStats(stats);
        setPostWheelNext("summary");
    } else {
        setPostWheelNext("streakFlash");
    }
    }  else {
      setPostWheelNext(null);
    }
...
const handleBonusAnswer = (correct) => {
  setBonusAnswerCorrect(correct);
  
  if (correct && selectedCharacter) {
    unlockCharacter(selectedCharacter);
    setTotalCoinsEarned(prev => prev + 10); // Bonus coins
  }
  
  // Prepare stats for summary
  const stats = {
    correctAnswers: runResults.filter(Boolean).length + (correct ? 1 : 0),
    totalQuestions: 4, // 3 regular + 1 bonus
    coinsEarned: totalCoinsEarned + (correct ? 10 : 0),
    xpEarned: runResults.filter(Boolean).length + (correct ? 2 : 0),
    perfectRound: runResults.every(Boolean),
    characterUnlocked: selectedCharacter ? WHEEL_CATEGORIES.find(c => toSlug(c.name) === selectedCharacter) : null,

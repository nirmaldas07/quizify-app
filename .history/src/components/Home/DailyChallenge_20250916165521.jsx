...
// Load questions on mount
  useEffect(() => {
    ...
    if (savedData) {
      ...
      const savedDate = new Date(parsed.completedAt).toDateString();
      const today = new Date().toDateString();
      
      if (savedDate === today) {
        // Already completed today, show complete state
        setStage('complete');
  ...
  const handleStart = () => {
    ...
    AnalyticsService.trackButtonClick('daily_challenge_start', 'home_screen');
    setStage('playing');
    setCurrentIndex(0);

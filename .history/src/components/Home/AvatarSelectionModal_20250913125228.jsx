          ...
          {GAME_CONSTANTS.AVATARS.map(avatar => {
            ...
            return (
              <button
                ...
                onClick={() => {
                if (!isLocked) {
                    AnalyticsService.trackButtonClick(`avatar_selected_${avatar.id}`, 'avatar_modal');
                    changeAvatar(avatar.id);
                    onClose();
                } else {
                    AnalyticsService.trackButtonClick(`avatar_locked_clicked_${avatar.id}`, 'avatar_modal');
                }
                }}

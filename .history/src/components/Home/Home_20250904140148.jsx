          ...
          <CoinFly
            ...
            onDone={() => {
              if (!dailyCoinsAdded) {
                ...
                // Record in GameDataService for daily tracking
                const currentUser = UserService.getCurrentUser();
                if (currentUser?.phone) {
                  // This will update the daily progress properly
                  GameDataService.addCoins(coinFly.amount, 'Daily Challenge');
                  GameDataService.addXP(DAILY_Q_XP);
                  
                  // Update quest progress for daily challenges if you have such quests

...
export default function Quiz() {
  ...
  useEffect(() => {
    ...
    // FIXED: Energy check ONLY for quiz mode, NOT for practice mode
    if (!isPractice && !energyDeducted) {
      const energyCost = 1; // FIXED: Should be 1 energy per quiz as per flow
      console.log("Energy check - Current energy:", energy, "Cost:", energyCost);
      
      if (energy < energyCost) {
        console.log("Not enough energy for quiz mode");
        navigate("/", { 
          state: { 
            message: "Not enough energy for Quiz mode. Try Practice mode instead!",
            openPractice: true // FIXED: Should open practice modal
          } 
        });
        return;
      }
      ...
      console.log("Deducting energy for quiz mode");

...
useEffect(() => {
  ...
  // Kick the progress animation + sound AFTER we're back on the wheel
useEffect(() => {
  if (gameState !== 'wheel' || pendingProgressIdx == null) return;

  // wait a tick so the wheel/progress bar is on screen
  const t0 = setTimeout(() => {
    setShowSparkle(pendingProgressIdx);
    if (soundOn) playSound('/sounds/progress.mp3', 0.6);
  }, 30);
  ...
  const t1 = setTimeout(() => setShowSparkle(-1), 900);
  const t2 = setTimeout(() => setPendingProgressIdx(null), 950);

  return () => { clearTimeout(t0); clearTimeout(t1); clearTimeout(t2); };
}, [gameState, pendingProgressIdx, soundOn]);
...
const stopSpinSound = () => {
  ...
  const handleAnswer = (correct, motivator, isContinue = false) => {
    if (isContinue) {
      ...
      // ✅ mark this step as attempted regardless of correctness
      newProgress[run.qIndex] = true;
      ...
      // defer progress animation/sound until we are back on the wheel
      setPendingProgressIdx(run.qIndex);

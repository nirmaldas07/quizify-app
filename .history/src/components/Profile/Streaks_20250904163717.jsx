...
// Define today at the top
  ...
  const handleClaimReward = (reward, day, e) => {
    ...
    // const buttonRect = e?.currentTarget?.getBoundingClientRect(); // Don't store button rect - we'll use gift box position instead
    ...
    // For current week logic
    if (isToday) {
        // Today - check if user has completed today's activity
        return currentStreak > 0 ? 'today-completed' : 'today-pending';
    } else if (weekIndex < weekDays.findIndex(d => d.dayIndex === todayIdx)) {
        // Past days in current week (before today)
        // Check if user maintained streak on those days
        // For simplicity, if current streak >= days passed, they're completed
        const daysPassed = weekDays.findIndex(d => d.dayIndex === todayIdx) - weekIndex;
        return currentStreak >= daysPassed ? 'completed' : 'missed';
    } else {

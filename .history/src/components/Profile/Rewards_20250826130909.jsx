...
const RewardsJourney = () => {
  ...
  const claimReward = (index, reward) => {
    ...
    if (reward.type === 'coins') {
      ...
      // For cards (vehicle, wildcard, etc.)
      setTimeout(() => {
        setFlyingCard({
          icon: reward.icon,
    ...
    // Auto-scroll and progress to next
    setTimeout(() => {
      ...
      if (currentProgress < rewards.length - 1) {
        ...
        // First move to next progress
        setTimeout(() => {
          setCurrentProgress(prev => prev + 1);
          setIsMoving(false);
          
          // Then auto-scroll after avatar has moved
          setTimeout(() => {
            const nextIndex = currentProgress + 1;
            const nextElement = journeyRef.current?.querySelector(`[data-index="${nextIndex}"]`);
            if (nextElement) {
              // Calculate position to keep element in center of viewport below sticky header
              const stickyHeaderHeight = 300; // Approximate height of sticky section
              const elementRect = nextElement.getBoundingClientRect();
              const scrollTop = window.pageYOffset || document.documentElement.scrollTop;
              const targetScroll = scrollTop + elementRect.top - stickyHeaderHeight - (window.innerHeight - stickyHeaderHeight) / 2;

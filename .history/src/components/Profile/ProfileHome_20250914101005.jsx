...
// Get user's first name
    ...
    // Get the latest name from profile data if it exists
    const profileData = currentUser.phone 
        ? JSON.parse(localStorage.getItem(`user_profile_${currentUser.phone}`) || '{}')
        : {};
    
    const fullName = profileData.name || currentUser.username || player?.name || "Player";
    ...
    const getTodayCoins = () => {
    // Use GameDataService summary if available, otherwise fallback to old method
    if (summary?.todayCoins !== undefined) {
        return summary.todayCoins;
    }
...
const userStats = {
  name: firstName,
  rank: summary?.rank || 23,
  streak: summary?.streak || player?.currentStreak || 0,
  todayQuizzes: summary?.daily?.quizzesPlayed || 3,
  ...
  // Load game data from GameDataService
  useEffect(() => {
    const loadGameData = () => {
      ...
      if (currentUser?.phone) {
        ...
        // Get today's summary
        const data = GameDataService.getTodaySummary(currentUser.phone);
        setSummary(data);
      }
    };
    ...
    window.addEventListener('focus', loadGameData);
    return () => window.removeEventListener('focus', loadGameData);
  }, []);
  ...
  return (
    ...
    {/* Enhanced Header with User Info */}
        <div className="mb-6">
            ...
            <div>
                <h1 className="text-2xl font-bold px-3">{greeting}, {firstName} 👋</h1>
            </div>

            {['+919701019117'].includes(UserService.getCurrentUser()?.phone) && (
            <button 
              onClick={() => navigate("/admin")}
              className="w-10 h-10 rounded-2xl bg-gradient-to-br from-purple-500/20 to-blue-500/20 border border-purple-500/30 grid place-items-center hover:from-purple-500/30 hover:to-blue-500/30 transition mr-2"
        ...
        <div className="rounded-3xl bg-gradient-to-r from-slate-800/50 to-slate-900/50 border border-white/10 p-4 backdrop-blur-sm">
          <div className="flex items-center gap-4 mb-3">
            ...
            <div className="flex-1">
             ...
             <div className="text-white/60 text-sm">Level {summary?.level || player.level || 1}</div>
            </div>
            <div className="text-right">
              <div className="text-yellow-400 font-bold text-lg">{summary?.totalCoins || player.coins || 0} 🪙</div>
          ...
          <div className="mb-3">
            <div className="flex justify-between text-xs text-white/60 mb-1">
              ...
              <span>{summary?.xpProgress || levelProgress.current}/{summary?.xpNeeded || levelProgress.required}</span>
            </div>

...
const handleSignOut = () => {
  ...
  // Save the phone to sessionStorage for PhoneAuth to use
  if (userPhone) {
    // Extract just the number without country code (assuming +91 or other country codes)
    const phoneWithoutCode = userPhone.replace(/^\+\d{1,3}/, '');
    sessionStorage.setItem('tempPhone', phoneWithoutCode);
    sessionStorage.setItem('tempCountryCode', userPhone.substring(0, userPhone.length - phoneWithoutCode.length));
  }
  
  // Navigate to PhoneAuth page - let it handle the redirect logic
  navigate('/auth', { replace: true, state: { phone: userPhone?.replace(/^\+\d{1,3}/, '') } });
};

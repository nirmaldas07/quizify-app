...
// Override "You" entries with actual player data - without useMemo to ensure fresh data
const dynamicLeaderboardData = {
  global: leaderboardData.global.map(entry => 
    entry.you ? { ...entry, name: player?.name || "You", score: player?.coins || 0, streak: player?.currentStreak || 0 } : entry
  ),
...
console.log("Debug - Player coins:", player?.coins);
console.log("Debug - Friends data:", dynamicLeaderboardData.friends[0]);
  ...
  console.log("Player coins:", player?.coins, "Current user score:", currentData.find(u => u.you)?.score);

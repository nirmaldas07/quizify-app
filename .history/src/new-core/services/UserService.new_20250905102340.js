// src/new-core/services/UserService.new.js
...
class UserService {
  ...
  getUserData(phone) {
    ...
    // Get user data by phone or username (overloaded version)
    getUserDataByIdentifier(identifier) {
        try {
        // First try as phone
        const userData = this.getUserData(identifier);
        if (userData) return userData;
        
        // Then try to find by username
        const users = JSON.parse(localStorage.getItem('users') || '{}');
        for (let phone in users) {
            if (users[phone].username === identifier) {
        ...
        console.error('Error in getUserDataByIdentifier:', error);
        return null;
        }
    }
  ...
  // Update user profile
    updateUserProfile(phone, updates) {
        ...
        if (updates.name) {
            ...
            // IMPORTANT: Preserve password
            const existingPassword = users[phone].password;
            users[phone].password = existingPassword;
            localStorage.setItem('users', JSON.stringify(users));
            }
        }
        ...
        // Update current user if logged in - FIXED
        const currentUser = JSON.parse(localStorage.getItem('currentUser') || '{}');
        if (currentUser.phone === phone) {
            currentUser.username = updates.name || currentUser.username;
            localStorage.setItem('currentUser', JSON.stringify(currentUser));
            
            // Also update game player
            const player = JSON.parse(localStorage.getItem('qp_player') || '{}');
            player.name = updates.name || player.name;
            localStorage.setItem('qp_player', JSON.stringify(player));
        }

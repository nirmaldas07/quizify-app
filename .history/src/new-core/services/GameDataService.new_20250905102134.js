// src/new-core/services/GameDataService.new.js
// SINGLE SOURCE OF TRUTH - Fixed to prevent coin duplication

import UserService from './UserService.new';
...
class GameDataService {
  constructor() {
    ...
    // FIX: Add flag to prevent duplicate updates
    this.isUpdatingCoins = false;
  ...
  // FIX: Main fix for coin duplication
  addCoins(amount, reason = '') {
    // Prevent duplicate updates
    if (this.isUpdatingCoins) {
      console.log(`[BLOCKED] Duplicate coin update: ${amount} from ${reason}`);
      return this.getCoins();
    }
    
    // Set flag immediately
    this.isUpdatingCoins = true;
    ...
    console.log(`[COINS] ${current} + ${amount} = ${newTotal} (${reason})`);

...
`&&k!=="\r"&&k!==`\r
`&&(k=`
`);var M=0,q=!1;this.parse=function(D,L,A){if(typeof D!="string")throw new Error("Input must be a string");var H=D.length,re=w.length,J=k.length,O=P.length,I=S(F),W=[],z=[],$=[],Z=M=0;if(!D)return ue();if(ee||ee!==!1&&D.indexOf(B)===-1){for(var ie=D.split(k),de=0;de<ie.length;de++){if($=ie[de],M+=$.length,de!==ie.length-1)M+=k.length;else if(A)return ue();if(!P||$.substring(0,O)!==P){if(I){if(W=[],ce($.split(w)),Ne(),q)return ue()}else ce($.split(w));if(Y&&Y<=de)return W=W.slice(0,Y),ue(!0)}}return ue()}for(var te=D.indexOf(w,M),ne=D.indexOf(k,M),K=new RegExp(C(_)+C(B),"g"),R=D.indexOf(B,M);;)if(D[M]===B)for(R=M,M++;;){if((R=D.indexOf(B,R+1))===-1)return A||z.push({type:"Quotes",code:"MissingQuotes",message:"Quoted field unterminated",row:W.length,index:M}),he();if(R===H-1)return he(D.substring(M,R).replace(K,B));if(B===_&&D[R+1]===_)R++;else if(B===_||R===0||D[R-1]!==_){te!==-1&&te<R+1&&(te=D.indexOf(w,R+1));var G=pe((ne=ne!==-1&&ne<R+1?D.indexOf(k,R+1):ne)===-1?te:Math.min(te,ne));if(D.substr(R+1+G,re)===w){$.push(D.substring(M,R).replace(K,B)),D[M=R+1+G+re]!==B&&(R=D.indexOf(B,M)),te=D.indexOf(w,M),ne=D.indexOf(k,M);break}if(G=pe(ne),D.substring(R+1+G,R+1+G+J)===k){if($.push(D.substring(M,R).replace(K,B)),$e(R+1+G+J),te=D.indexOf(w,M),R=D.indexOf(B,M),I&&(Ne(),q))return ue();if(Y&&W.length>=Y)return ue(!0);break}z.push({type:"Quotes",code:"InvalidQuotes",message:"Trailing quote on quoted field is malformed",row:W.length,index:M}),R++}}else if(P&&$.length===0&&D.substring(M,M+O)===P){if(ne===-1)return ue();M=ne+J,ne=D.indexOf(k,M),te=D.indexOf(w,M)}else if(te!==-1&&(te<ne||ne===-1))$.push(D.substring(M,te)),M=te+re,te=D.indexOf(w,M);else{if(ne===-1)break;if($.push(D.substring(M,ne)),$e(ne+J),I&&(Ne(),q))return ue();if(Y&&W.length>=Y)return ue(!0)}return he();function ce(ge){W.push(ge),Z=M}function pe(ge){var xe=0;return xe=ge!==-1&&(ge=D.substring(R+1,ge))&&ge.trim()===""?ge.length:xe}function he(ge){return A||(ge===void 0&&(ge=D.substring(M)),$.push(ge),M=H,ce($),I&&Ne()),ue()}function $e(ge){M=ge,ce($),$=[],ne=D.indexOf(k,M)}function ue(ge){if(g.header&&!L&&W.length&&!U){var xe=W[0],ye=Object.create(null),ht=new Set(xe);let Qt=!1;for(let at=0;at<xe.length;at++){let ke=xe[at];if(ye[ke=S(g.transformHeader)?g.transformHeader(ke,at):ke]){let ze,At=ye[ke];for(;ze=ke+"_"+At,At++,ht.has(ze););ht.add(ze),xe[at]=ze,ye[ke]++,Qt=!0,(Q=Q===null?{}:Q)[ze]=ke}else ye[ke]=1,xe[at]=ke;ht.add(ke)}Qt&&console.warn("Duplicate headers found and renamed."),U=!0}return{data:W,errors:z,meta:{delimiter:w,linebreak:k,aborted:q,truncated:!!ge,cursor:Z+(L||0),renamedHeaders:Q}}}function Ne(){F(ue()),W=[],z=[]}},this.abort=function(){q=!0},this.getCharIndex=function(){return M}}function h(g){var w=g.data,k=l[w.workerId],P=!1;if(w.error)k.userError(w.error,w.file);else if(w.results&&w.results.data){var F={abort:function(){P=!0,x(w.workerId,{data:[],errors:[],meta:{aborted:!0}})},pause:E,resume:E};if(S(k.userStep)){for(var Y=0;Y<w.results.data.length&&(k.userStep({data:w.results.data[Y],errors:w.results.errors,meta:w.results.meta},F),!P);Y++);delete w.results}else S(k.userChunk)&&(k.userChunk(w.results,F,w.file),delete w.results)}w.finished&&!P&&x(w.workerId,w.results)}function x(g,w){var k=l[g];S(k.userComplete)&&k.userComplete(w),k.terminate(),delete l[g]}function E(){throw new Error("Not implemented.")}function N(g){if(typeof g!="object"||g===null)return g;var w,k=Array.isArray(g)?[]:{};for(w in g)k[w]=N(g[w]);return k}function T(g,w){return function(){g.apply(w,arguments)}}function S(g){return typeof g=="function"}return u.parse=function(g,w){var k=(w=w||{}).dynamicTyping||!1;if(S(k)&&(w.dynamicTypingFunction=k,k={}),w.dynamicTyping=k,w.transform=!!S(w.transform)&&w.transform,!w.worker||!u.WORKERS_SUPPORTED)return k=null,u.NODE_STREAM_INPUT,typeof g=="string"?(g=(P=>P.charCodeAt(0)!==65279?P:P.slice(1))(g),k=new(w.download?p:b)(w)):g.readable===!0&&S(g.read)&&S(g.on)?k=new v(w):(s.File&&g instanceof File||g instanceof Object)&&(k=new m(w)),k.stream(g);(k=(()=>{var P;return!!u.WORKERS_SUPPORTED&&(P=(()=>{var F=s.URL||s.webkitURL||null,Y=r.toString();return u.BLOB_URL||(u.BLOB_URL=F.createObjectURL(new Blob(["var global = (function() { if (typeof self !== 'undefined') { return self; } if (typeof window !== 'undefined') { return window; } if (typeof global !== 'undefined') { return global; } return {}; })(); global.IS_PAPA_WORKER=true; ","(",Y,")();"],{type:"text/javascript"})))})(),(P=new s.Worker(P)).onmessage=h,P.id=c++,l[P.id]=P)})()).userStep=w.step,k.userChunk=w.chunk,k.userComplete=w.complete,k.userError=w.error,w.step=S(w.step),w.chunk=S(w.chunk),w.complete=S(w.complete),w.error=S(w.error),delete w.worker,k.postMessage({input:g,config:w,workerId:k.id})},u.unparse=function(g,w){var k=!1,P=!0,F=",",Y=`\r
